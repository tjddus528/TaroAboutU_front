...
function ResultLove_inven({route}){
    ...
    useEffect(() => {
        const userId = 1; //temp
        axios
        .get(`https://csyserver.shop/inventory/tarot/${userId}?questionId=3&date=${date}`)
        .then((res) => {
        ...
        <View style={styles.block}>
            <TouchableOpacity
                onPress={()=>{
                    ...
                    navigation.navigate('ResultLove', {
                        cardId: item.tarotId, 
                        isSaved: true
                    });
                }}>
       ...
       <View style={styles.container}>
           <ImageBackground source={require('../../img/background.png')} style={{width:"100%",height:"102%",top:-20}}>
                   ...
                   <View style={styles.result}>
                       <Text style={{color:"blanchedalmond",fontSize:30,marginBottom:-20,marginTop:90}}>연애 타로</Text>
